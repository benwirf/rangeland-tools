...
class TotalGrowthSummary(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        for district_name in district_names:
            ...
            count_results = self.total_growth_counts(raster1, region, northern_value_list, southern_value_list)
            feat = QgsFeature()
            feat.setAttributes([
                    district_name,
                    int(count_results[0]),
...
##############################################################################
    ...
    def total_growth_counts(self, raster, region, northern_scale_vals, southern_scale_vals):
        if region == 'Northern':
            # Northern Districts
